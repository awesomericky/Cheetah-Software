...
#include "FSM_State_DAGGERJointPD.h"
 ...
 */
template <typename T>
FSM_State_DAGGERJointPD<T>::FSM_State_DAGGERJointPD(ControlFSMData<T>* _controlFSMData)
    : FSM_State<T>(_controlFSMData, FSM_StateName::DAGGER_JOINT_PD, "DAGGER_JOINT_PD"),
...
template <typename T>
void FSM_State_DAGGERJointPD<T>::onEnter() {
  std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
 ...
 */
template <typename T>
void FSM_State_DAGGERJointPD<T>::run() {
  ...
  if(std::acos(this->_data->_stateEstimator->getResult().rBody.transpose().row(2)(2)) > 3.1415*30./180.) {
    ...
    std::cout << "DAGGER Joint PD Control mode is denied!!!" << std::endl;
    emergency_stop = true;
    return;
  }
 ...
 */
template <typename T>
FSM_StateName FSM_State_DAGGERJointPD<T>::checkTransition() {
  this->nextStateName = this->stateName;
 ...
 */
template <typename T>
TransitionData<T> FSM_State_DAGGERJointPD<T>::transition() {
  // Switch FSM control mode
 ...
 */
template <typename T>
void FSM_State_DAGGERJointPD<T>::onExit() {
  // Nothing to clean up when exiting
}

// template class FSM_State_JointPD<double>;
template class FSM_State_DAGGERJointPD<float>;

template <typename T>
const Eigen::Matrix<float, DAGGEROBSDIM, 1> & FSM_State_DAGGERJointPD<T>::getObservation() {
  _obs <<
